    ...
    public void onBotMsgRecieved(MessageReceivedEvent e)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < descs.length; i++)
            {
                ...
                GitHubAPI api = new GitHubAPI("****************************************", "***********", "****************", "main");

                if (!embed.getTitle().contains(api.repo)) return;

                StringBuilder builder = new StringBuilder();

                for (String filename : api.getValues("filename"))
                {
                ...
                int fileCount = api.getValues("filename").length;
                ...
                EmbedBuilder embedBuilder = new EmbedBuilder()
                        .withTitle("ファイルが更新されました")
                        .appendDesc("```" + desc + "```\n")
                        .appendDesc(":scroll: **ファイル**： " + builder.toString().substring(0, builder.toString().length() - 2) + "\n\n")
                        .appendDesc(":link: **詳細**： [" + commitID + "](" + url + ")\n\n")
